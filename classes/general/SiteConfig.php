        ...
        switch ($item['type']) {
            ...
            case 'file':
                ...
                if (!empty($item['value'])) {
                    ...
                    $result .= '<br /><a href="/upload/' . $file['SUBDIR'] . '/' . $file['FILE_NAME'] . '" target="_blank">' . $file['ORIGINAL_NAME'] . ' (' . $file['FILE_SIZE'] . ' b)</a>&nbsp;&nbsp;&nbsp;<label><input style="position:relative;top:0.25em;" type="checkbox" name="' . $item['name'] . '__del" value="1" /> Удалить файл</label>';
                }
                break;
        ...
        foreach ($xmlData as $section) {
            foreach ($section as $item) {
                ...
                switch ($item['type']) {
                    ...
                    case "checkbox":
                        $item['value'] = isset($_POST[$name]) ? 1 : 0;
                        break;
